...
for epoch in range(opt.nepoch):
    ...
    if epoch <0:
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            #真实值
            label_R=GenNetwork.foward(points,Points_noise,rand_id)
            label_R=label_R.transpose(2, 1)
            errD_real = 0.5 * torch.mean((label_R - 1) ** 2)  # criterion(output, label)
            #errD_real.backward()
            #扰动值
            label_F = GenNetwork.foward(points,pointsReconstructed,rand_id)  # Create fake image
            ...
            dist1_rig, dist2_rig = distChamfer(points_clone.transpose(2, 1).contiguous(), pointsReconstructed_RIG)
            ...
            regul = laplaceloss(pointsReconstructed)
            dist1, dist2 = distChamfer(points_clone.transpose(2, 1).contiguous(), pointsReconstructed)
            
            dist, indx = knn(points_clone.transpose(2, 1),pointsReconstructed)
            indx_t = indx.view(opt.batchSize, 6890).unsqueeze(-1).expand(opt.batchSize, 6890, 3).cuda()
            #pointsReconstructed = pointsReconstructed.gather(1, indx_t)
            points_clone = points_clone.transpose(2, 1).gather(1, indx_t)
            label_G = GenNetwork.foward(points_clone,pointsReconstructed,rand_id)  # Create fake images
