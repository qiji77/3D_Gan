...
def Tran_points_rig(Translate_matrix, template_points_toTra, train_sig, Rig_list):
    ...
    #Point_R[:, Rig_list, :] = torch.mean(Point_R_clone[:, Rig_list, :], dim=1, keepdim=True)
    #Point_theat[:, Rig_list, :] = torch.mean(Point_theat_clone[:, Rig_list, :], dim=1, keepdim=True)
    #Point_T[:, Rig_list, :] = torch.mean(Point_T_clone[:, Rig_list, :], dim=1, keepdim=True)
