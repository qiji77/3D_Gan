...
class LocalSpatialEncoding(nn.Module):
    ...
    def forward(self, coords, features, knn_output):
        ...
        extended_idx=extended_idx.cuda()
        ...
        #print("local s time is",localend-localstime)
        return torch.cat((
            self.mlp(concat),
...
class LocalFeatureAggregation(nn.Module):
    ...
    def forward(self, coords, features):
        ...
        knn_output = knn1(coords.cuda().contiguous(), coords.cuda().contiguous())
...
class RandLANet(nn.Module):
    def __init__(self, d_in, num_classes, num_neighbors=8, decimation=8, device=torch.device('cuda')):
    ...
    def forward(self, input):
        ...
        for mlp in self.decoder:
            ...
            _,neighbors= knn(
                coords[:,:N//decimation_ratio].cuda().contiguous(), # original set
                coords[:,:d*N//decimation_ratio].cuda().contiguous() # upsampled set
            ) # shape (B, N, 1)
